...
#define DEBUGVHDL 0
	...
	Posit2UInt::Posit2UInt(OperatorPtr parentOp, Target* target, int widthI, int wESI, int widthO, bool trunc_p) :
        ...
        srcFileName="Posit2UInt";

		name<<"Posit2UInt_" << width_ << "_" << wES_ << "_to_" << widthO_ << "_" << (trunc_p == true ? "T" : "NT");
        setNameWithFreqAndUID(name.str());

		if (width_ < 3)
		{
			throw std::string("Posit2UInt Constructor: width is too small, should be greater than two");
		}
		if (wES_ >= width_ - 3)
		{
			//Avoid posits without even one bit of precision
			throw std::string("Posit2UInt Constructor: invalid value of wES");
		}
        ...
        unsigned int intExp = widthO_;
        bool possibleOverflow = (positExp >= intExp); // Max int value is 2**(widthO_) - 1
        if (possibleOverflow){
        ...
        if (shifterSize >= (wF_+1)){
            vhdl << tab << declare("paddedFrac", shifterSize) << " <= '1' & shiftedPosit" << range(wF_ - 1, 0) << " & " << zg(shifterSize - (wF_+1)) << ";" << endl;
        }
        else {
            int remBits = (wF_+1) - shifterSize;
            vhdl << tab << declare(getTarget()->eqConstComparatorDelay(remBits), "stkFrac", 1, false) << " <= '0' when shiftedPosit" << range(remBits-1, 0) << "=0 else '1';" << endl;
            vhdl << tab << declare("paddedFrac", shifterSize) << " <= '1' & shiftedPosit" << range(wF_ - 1, remBits) << ";" << endl;
        }
        ...
        if (possibleOverflow){
            ...
            vhdl << tab << declare(getTarget()->adderDelay(sizeRightShift), "shiftVal", sizeRightShift) << " <= "
                << "\"" << unsignedBinary(widthO_-1, sizeRightShift) << "\" - sf";
            if (sizeRightShift > wE_)
            {
                vhdl << "Extended";
            }
        ...
        vhdl << tab << declare("padBit", 1, false) << " <= '0';" << endl; 
        if (trunc_p){
            newInstance("Shifter",
                        "RightShifterComponent",
                        "wX=" + to_string(shifterSize) + " wR=" + to_string(shifterSize) + " maxShift=" + to_string(widthO_) + " dir=1 inputPadBit=true",
                        "X=>paddedFrac, S=>shiftVal, padBit=>padBit",
                        "R=>shiftedFrac");
            
            vhdl << tab << declare("intNumber", widthO_) << " <= shiftedFrac;" << endl;
        }
        else{
            newInstance("Shifter",
                        "RightShifterComponent",
            ...
            if (shifterSize >= (wF_+1)){
                vhdl << tab << declare("sticky", 1, false) << " <= stk;" << endl;
            }
            else{
                vhdl << tab << declare(getTarget()->logicDelay(2), "sticky", 1, false) << " <= stk OR stkFrac;" << endl;
            }

            vhdl << tab << declare("rnd", 1, false) << " <= shiftedFrac" << of(0) << ";" << endl;
            vhdl << tab << declare("lsb", 1, false) << " <= shiftedFrac" << of(1) << ";" << endl;
            vhdl << tab << declare(getTarget()->logicDelay(3), "round", 1, false) << " <= rnd AND (lsb OR sticky);" << endl;
            
            vhdl << tab << declare(getTarget()->adderDelay(widthO_), "intNumber", widthO_) << " <= shiftedFrac" << range(shifterSize-1, 1) << " + round;" << endl;
        }
	...
	Posit2UInt::~Posit2UInt() {
	}

    void Posit2UInt::emulate(TestCase * tc){
    }

	OperatorPtr Posit2UInt::parseArguments(OperatorPtr parentOp, Target *target, vector<string> &args)
	{
		...
		return new Posit2UInt(parentOp, target, widthI, wES, widthO, trunc);
	}

	void Posit2UInt::registerFactory()
	{
		UserInterface::add("Posit2UInt", // name
