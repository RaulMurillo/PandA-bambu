...
#define DEBUGVHDL 0
	...
	Posit2Int::Posit2Int(OperatorPtr parentOp, Target* target, int widthI, int wESI, int widthO, bool trunc_p) :
        ...
        if (shifterSize >= (wF_+1)){
            vhdl << tab << declare(getTarget()->logicDelay(1), "paddedFrac", shifterSize) << " <= NOT(sgn) & shiftedPosit" << range(wF_ - 1, 0) << " & " << zg(shifterSize - (wF_+1)) << ";" << endl;
        }
        else {
            ...
            vhdl << tab << declare(getTarget()->logicDelay(1), "paddedFrac", shifterSize) << " <= NOT(sgn) & shiftedPosit" << range(wF_ - 1, remBits) << ";" << endl;
        }
