...
namespace flopoco
{
					   ...
					   int wFF) : Operator(parentOp, target), widthP_(widthP), wESP_(wESP), wEF_(wEF), wFF_(wFF)
	{
		...
		if (wEF_ < wE_)
		{
			REPORT(DEBUG, "WARNING - Posit2FP Constructor : exponent field length of the output is too small, possible range issues");
										 ...
										 std::vector<std::string> &args)
	{
		...
		UserInterface::parsePositiveInt(args, "wES", &wESP);
	...
	void Posit2FP::registerFactory()
	{
		UserInterface::add("Posit2FP",
						   ...
						   wES(int): posit exponent field length;\
